    ...
    defaults: {
        ...
        health: {
            ...
            response: {
                ...
                sha: process.env.VCS_REF || '0000000',
                buildDate: process.env.BUILD_DATE || '1970-01-01T00:00:00Z',
                nodeVer: process.versions.node,
                npmVer: (process.env.npm_config_user_agent || 'npm/0.0.0')
                    .split(' ')
                    .reduce((result, value) => {
                        if (result === '0.0.0') {
...
exports.plugin = {
    ...
    register: (server, options) => {
        ...
        const validatedSettings = exports.validateSettings(settings);
        ...
        const initialisedTimers = initialisedMetrics
            ...
            .map((metric) => {
                return setInterval(() => metric.record(), validatedSettings.interval).unref();
            });
        ...
        validatedSettings.health.path.forEach((path) => {
            server.route({
                method: 'GET',
                path,
                options: {
